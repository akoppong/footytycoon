...
- Navy toolbar and navigation, ivory reading surfaces, square ruled panels, amber focus and selected states, and serif headings. Body text and supporting labels use regular Archivo (400); the two primary amounts use heavy Archivo (750). Acquisition highlights the full asking price and club cash; other screens highlight cash and personal reserve in the toolbar. IBM Plex Mono is reserved for aligned numeric table cells. All three fonts are bundled with their OFL licenses for offline use.
- Supporting labels and chart annotations start at 12px and scale with the text-size setting. Muted text uses `#5c6472` on ivory and `#b5c3d4` on navy; chart warning text uses `#a4361f`. The theme uses numeric OpenType axis tags because StringName keys left the bundled Archivo at its default weight of 600. The engine smoke probe checks the applied regular and heavy weights.
