...
The main lesson from the market comes from Football Manager 26 (November 2025). The most established game in the genre rebuilt its interface around tiles, widgets and pop-ups. Its Steam rating fell to about 22–23% positive, and the complaints focused on extra clicks, wasted screen space, league tables and statistics buried several screens deep, and no UI scaling. The patches players liked most made the interface *faster* (for example, an option to turn off transitions). They didn't add more visuals. So we should be **dramatic in identity and feedback, and conservative in navigation and data layout.**
...
This review is based on the source, not screenshots. The UI is built in code (`src/FootballTycoon.Desktop/Main.cs`, `Main.Design.cs`, `SeasonChart.cs`, about 1,300 lines).
...
|---|---|---|---|
| P1 | Developer notes appear in the player's UI. | "Prototype scope: …" (`Main.cs:100`). "Appointments are fixed in this prototype… future systems" (`Main.cs:179`). "This milestone supports three seasons" (`Main.Design.cs:316`). | This breaks immersion and makes the game look unfinished. It belongs in the docs, not the club dossier. |
| P2 | Most workspaces are prose, not structure. | Business prints the 52-week forecast as one text line every four weeks (`Main.cs:165`). The squad is a list of labels (`Main.cs:180`). The Club screen is hard-coded paragraphs (`Main.cs:95`). | Players have to read to find answers they should be able to spot, which is the opposite of the "find the main risk in 30 seconds" goal (PRD H2). |
...
| P6 | Text scaling is limited and only partly applied. | `Theme.DefaultFontSize = 14 * textScale / 100`, capped at 150% (`Main.cs:39`). Sizes are fixed per call and chart fonts are drawn separately. | Falls short of the ~200% expectation, and the chart and labels can drift out of step (the Frostpunk 2 problem). |
...
|---|---|---|---|---|
| A1 | **Remove developer notes from the player UI.** Move prototype disclaimers to docs or a single "About this build" line in Settings. Unimplemented systems simply don't appear in the UI. | P1 | Yes | S |
| A2 | **Always-visible status strip:** league position, form (W/D/L chips with letters as well as colour), club cash, headroom above reserve, next fixture with crest and date. Each item opens its detail with one click. | Pillar 2 and the FM26 lesson. The toolbar already shows cash and reserve, so this extends it. | Yes | S |
| A3 | **Inbox discipline:** at most three items that need a decision, related items grouped into one situation (injury, replacement proposal and cash effect together, as PRD H5 asks), and a separate "news" digest that never outranks decisions. Acknowledged notices don't repeat. | FM spam, Cities: Skylines II alert fatigue | Partly: grouping rules are needed | M |
| A4 | **"Where did I commit that?"** Every active commitment gets a permanent row in a Commitments list (Business) that links to its original terms, current effect and next review date. | The Victoria 3 lost-control problem | Yes (`History`, `Obligations`) | S |
| A5 | **A command palette (Ctrl+K) and global shortcuts** for the six workspaces (1–6), in addition to the existing Tab focus order. | Fewer clicks for experienced players. Keyboard-first players are core to this genre. | Yes | S |
...
| C3 | **Ground card.** | A stylised stadium silhouette with stands as separate layers, so later facility upgrades can light up a new stand (supporting G9). Capacity and average attendance appear next to it. | M |
| C4 | **Generated portraits** for staff and players. | Flat, abstract illustrated busts built from layered shapes (skin, hair, kit colours) using the existing `identity/{club}` random stream. Consistent and non-photographic, so there's nothing to license. Staff get hand-tuned variants. | L |
| C5 | **Club dossier replaces the hard-coded Club prose.** | Crest, founding line, town, rival (with the rival's crest), identity traits, ground card and "since you arrived" numbers (seasons, promotions, cup runs, cash change). | S (after C1–C3) |

### D. Show numbers as pictures (Phase 3)

| ID | Proposal | Replaces | Effort |
|---|---|---|---|
| D1 | **Business cash chart:** reuse `SeasonChart` at full width with the base/downside band and signed obligations stacked underneath. Hover or keyboard shows the exact week. The text list becomes an optional "table view" toggle, which also serves screen-reader and export needs. | The forecast list at `Main.cs:165` | M |
...
| D4 | **League table upgrade:** crests, form chips, promotion and relegation zone bands (as edges, not just colour), movement arrows since last week, and the table filtered to the teams around yours. | The current grid | S |
...
1. **Pre-match card** (about 3 seconds, or instant): both crests, league positions, form chips, unavailable players shown as chips, stakes ("win moves you into the promotion places"), and venue and date. Expected competitiveness is shown only as a qualitative band, never as a win percentage.
2. **Timeline ticker** (15–30 seconds, skippable, adjustable speed, "instant result" always available): a horizontal 90-minute bar with a live score. The 3–6 recorded moments land on it as they happen (goal, card, injury), with the scorer's chip and a short crowd sound cue. The simulation has already resolved the result, and the ticker only reveals it.
3. **Final card:** big score, shots, player of the match with rating, a small pitch diagram of the line-up coloured by rating (the rating shown as a number too), attendance against capacity, receipts as a `MoneyFigure`, the change in league position, and new injuries or bans. The manager's quote stays.
4. **Batching:** when Continue skips several weeks, results arrive as a single results reel (one row per match, and a click opens the full card), not N modals. Only matches that decide something or are marked as significant (cup ties, derbies, promotion or relegation six-pointers) get the ticker automatically. A setting controls which matches get the ticker: all, key matches only, or none.
5. **Variety guard:** the pre-match strapline draws from several templates chosen by context (derby, cup, run-in, winning streak) and avoids repeating in consecutive matches. This is the fix for the EA FC problem.

Effort: **L**. It's the most visible single improvement and belongs straight after the component layer, because matchday is shown in trailers and it's where the PRD's "football makes money visible" promise is kept or lost.

### F. Story and memory (Phase 4)
...
| F2 | **Season annual report:** a designed spread with final position (and a line chart of the season's position), cup run, the season's top performer, cash then and now as a waterfall, decisions made, each with its original forecast against actual result (PRD H5), and a closing line from the CEO. It replaces the plain season summary card. | M |
...
| F4 | **Records and callbacks:** club records (biggest win, top scorer, record attendance, longest-serving player). New events refer back to them where true ("his 50th appearance", "first promotion since your arrival"). This is presentation of stored data, not a new simulation system. | M |
...
- **Milestone ceremonies** (promotion, relegation, a cup win, an ownership milestone): a full-screen card with the crest, date, key numbers and one line from the CEO. It can be dismissed with any key, never shows more than once per event, and is logged in History. PRD H4 asks for this.
...
- **Motion rules:** a 150–250 ms ease for panel changes, no bounce, and nothing that loops forever. Reduced motion (on by default until everything is tested) turns every animation into an instant change. There's also a separate "UI transitions" toggle, the setting FM26 players praised most.
...
| Item | Target |
|---|---|
| UI scaling | 80–200% in steps of 10, applied to the whole interface (fonts *and* spacing, icons and the chart's drawn text), not just `Theme.DefaultFontSize`. |
| Minimum text | 12 px at 1280×720 and 1280×800 (the Deck-safe target), everywhere including tooltips and chart annotations. |
...
| Input | Full keyboard support (as today), remappable shortcuts, controller-friendly focus order (for Deck), and no action that only works on hover. |
...
- **Scaling afterthought:** no new component merges unless it passes the 200% and 1280×800 capture checks.
...
| Hat | View |
|---|---|
| **White (facts)** | The simulation is solid and CI is green. There's no audio or motion, 0 crests, and developer notes in the UI. FM26's UI reception is documented. No player has tested the current build (see DELIVERY_PROGRESS "Product proof"). |
...
| **Blue (process)** | Put component plumbing (B) before visual work. Pair each phase with a five-person player test against PRD M1 and M4 prompts. Stop and reassess after Phase 3 if comprehension doesn't improve. |
...
|---|---|---|
| **1 · Friction** | A1–A5, H (text scaling applied to all text, 200% cap, contrast test) | No developer notes in the UI. Every workspace fact reachable in two clicks or fewer. Smoke captures pass at 200% and 1280×800. |
| **2 · Foundation and identity** | B (components and tokens), C1, C2, C3, C5, F5 | Every club mention shows a crest. No `Label(...)` rebuilds in the converted screens. Scroll position and focus are kept on refresh. |
...
| **3 · Honest pictures** | D1–D7 | Players answer "where did the money go?" and "what happens if this goes badly?" without reading prose. |
...
2. **Portraits:** commit to generated portraits (C4), or decide now that initials-only chips are the house style?
3. **Steam Deck:** make 1280×800 and controller focus a formal target (affects H and every layout), or stay Windows desktop-first?
