...
| **Football Manager 26 patches** (2025–26) | Among the most welcomed fixes were speed improvements: much less lag, an option to turn off or slow UI transitions, and reworked navigation. | Every animation needs an off switch, and every screen should appear instantly. Speed is part of presentation. |
...
- Steam Deck Verified requires text of at least 9 px at 1280×800, and 12 px is a safer target. Small text is a common reason submissions fail. The PRD targets Windows desktop. Deck support is optional (Decision 3), but these thresholds are a useful readability benchmark either way.
...
|---|---|---|---|
| P1 | Developer notes appear in the player's UI. | "Prototype scope: …" (`Main.cs:100`). "Appointments are fixed in this prototype… future systems" (`Main.cs:179`). "This milestone supports three seasons" (`Main.Design.cs:316`). "Three-season milestone" in the status bar (`Main.cs:47`). The status banner prints raw enum names such as "PROTOTYPECOMPLETE" and "LOSTCONTROL" (`Main.Design.cs:172–173`). | This breaks immersion and makes the game look unfinished. It belongs in the docs, not the club dossier. |
...
| P6 | Text scales, but layout does not. | Text sizes and the chart's font and height scale with the setting, which is limited to 100/125/150% (`Main.cs:35`, settings at `Main.cs:249`). Padding, the 170 px button minimum width (`Main.cs:294`), navigation margins and the chart's fixed plot margins do not scale. Panel widths change only at exactly 150% (`Main.Design.cs:183–185`), and the three-column plan grid exists only at 100% (`Main.Design.cs:354`). | Larger text squeezes into fixed spacing. Anything above 150%, which is common in 2025–26 games, would need layout work. |
...
| A5 | **A command palette (Ctrl+K) and global shortcuts** for the six workspaces (1–6), in addition to the existing Tab focus order. Continue can currently be bound to a digit (rebinding check at `Main.cs:269`), so rebinding must reject keys already used by these shortcuts. | Fewer clicks for experienced players. Keyboard-first players are core to this genre. | Yes | S |
...
- **E5 · Variety guard:** the pre-match strapline draws from several templates chosen by context (cup, run-in, winning streak, and derby once rivalries exist per C0) and avoids repeating in consecutive matches. This is the fix for the EA FC problem.
