...
Each new season generates 720 fixtures, resets standings and asks for a new capital plan. Original decisions, monthly reviews, results and closing tables remain in the saved world. History shows closing reports; Football and the season chart show the current season. The interface shows dates rather than week numbers: season one is 2026/27, runs from late June to late June, and plays its league from mid-August to mid-May (see [dated calendar](CALENDAR.md)). Internally, contracts, forecasts and saved history still use cumulative career weeks (season two is career weeks 52–104). A renewal preview shows the upcoming season’s cash path; fixtures appear after confirmation.
...
Schema-1 saves migrate in memory through the current schema 6; original snapshots remain unchanged. Completed old saves open at the first season review. Confirming a renewal follows the same durable candidate-write rule as other decisions, and loading preserves branch ancestry.
